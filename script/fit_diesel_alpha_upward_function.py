"""
Fit and plot the diesel upward alpha approximation function.

The fitted function is intentionally simple and interpretable:

alpha_d^up = clip[
    beta0
    + beta1 * log(1 + max(f_d - k_d, 0))
    + beta2 * max(f_g - k_g, 0)
    + beta3 * S_d(t-1)
    + beta4 * u_d(t-1)
...
from diesel_alpha_upward_adjustment_function import diesel_alpha_upward_adjustment


ROOT = Path(__file__).resolve().parent.parent
RESULT_DIR = ROOT / "result"
FUNCTION_FILE = ROOT / "script" / "diesel_alpha_upward_adjustment_function.py"

PARTIAL_EFFECT_FILE = RESULT_DIR / "task2_alpha_partial_effects.csv"
SCENARIO = "upward_base"
DIESEL_F_THRESHOLD = 40.0
GASOLINE_F_THRESHOLD = 0.0
MODELED_VARIABLES = [
    "f_diesel",
    "f_gasoline",
    "S_diesel_lag",
        ...
        rows.append(
            {
                ...
                "alpha_target": float(row.alpha_diesel),
                **features,
            }
        )
...
def make_params(theta: np.ndarray, base: dict[str, float]) -> dict[str, float]:
    ...
    params.update(
        {
            "fd_threshold": DIESEL_F_THRESHOLD,
            "fg_threshold": GASOLINE_F_THRESHOLD,
            "cpi_low": 0.02,
            "cpi_high": 0.03,
            "fd_log_center": float(np.log1p(max(base["f_diesel"] - DIESEL_F_THRESHOLD, 0.0))),
            "fg_hinge_center": float(max(base["f_gasoline"] - GASOLINE_F_THRESHOLD, 0.0) / 100.0),
            "S_d_center": float(base["S_diesel_lag"]),
            "prev_u_d_center": float(base["prev_u_diesel"]),
...
def predict_frame(data: pd.DataFrame, params: dict[str, float]) -> np.ndarray:
    return np.array(
        [
            diesel_alpha_upward_adjustment(
...
def fit_model(train: pd.DataFrame, base: dict[str, float]) -> tuple[dict[str, float], pd.DataFrame]:
    ...
    lower = np.array([0.50, -0.40, 0.00, 0.00, 0.00, -0.40, 0.00])
    upper = np.array([1.00, 0.00, 0.20, 0.20, 0.20, 0.20, 0.20])
    initial = np.array([0.90, -0.02, 0.02, 0.02, 0.02, -0.03, 0.03])
    fit = least_squares(
        residual,
        initial,
        bounds=(lower, upper),
...
def save_parameter_outputs(params: dict[str, float], fit_metrics: dict[str, float]) -> None:
    ...
    pd.DataFrame(rows).to_csv(
        RESULT_DIR / "task2_alpha_diesel_upward_function_params.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def update_function_file(params: dict[str, float]) -> None:
    ...
    pattern = r"DIESEL_UPWARD_ADJUSTMENT_ALPHA_PARAMS = \{.*?\n\}"
    replacement = f"DIESEL_UPWARD_ADJUSTMENT_ALPHA_PARAMS = {formatted}"
            ...
            rows.append(
                {
                    ...
                    "x_value": float(row.x_value),
                    "alpha_target": float(row.alpha_diesel),
                    "alpha_fitted": diesel_alpha_upward_adjustment(**features, params=params),
                }
            )
    return pd.DataFrame(rows)
...
def plot_overlay(overlay: pd.DataFrame) -> Path:
    ...
    for ax, variable in zip(axes, MODELED_VARIABLES):
        ...
        ax.set_xlabel("")
        ax.set_ylabel("diesel alpha")
        ax.grid(True, alpha=0.25)

    axes[-1].axis("off")
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper right", bbox_to_anchor=(0.96, 0.96))
    fig.suptitle("Diesel upward alpha function: partial-effect fit check", y=0.98)
    fig.tight_layout(rect=[0, 0, 0.98, 0.95])
    path = RESULT_DIR / "task2_alpha_diesel_upward_function_overlay.png"
...
def main() -> None:
    ...
    fitted_train.to_csv(
        RESULT_DIR / "task2_alpha_diesel_upward_function_fit_data.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    overlay.to_csv(
        RESULT_DIR / "task2_alpha_diesel_upward_function_overlay_data.csv",
        index=False,
        encoding="utf-8-sig",
    )
    plot_path = plot_overlay(overlay)

    print("Fitted diesel upward alpha function")
    print(pd.DataFrame([params]).T.rename(columns={0: "value"}).to_string())
    print("\nMetrics")
    print(pd.DataFrame([fit_metrics]).to_string(index=False))
    print("\nOutputs")
    print("- task2_alpha_diesel_upward_function_params.csv")
    print("- task2_alpha_diesel_upward_function_fit_data.csv")
    print("- task2_alpha_diesel_upward_function_overlay_data.csv")
    print(f"- {plot_path.name}")
    print("- script/diesel_alpha_upward_adjustment_function.py")
